...
:setvar schema "dbo"
:setvar name "GetPeriodBeginByDate"
...
AS
BEGIN
  RETURN DATEADD(MONTH, @Shift * (12 / @Frequency), DATEFROMPARTS(YEAR(@Date), (MONTH(@Date) / (12 / @Frequency)) * (12 / @Frequency) + IIF(MONTH(@Date) % (12 / @Frequency) <> 0, 1, 1 - (12 / @Frequency)), 1))
END
GO

/* Использование */
SELECT dbo.GetPeriodBeginByDate(GETDATE(), 4, 0)
